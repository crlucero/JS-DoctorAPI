  ...
  $("form#drSearch").submit(function() {
    ...
    promise.then(function(response) {
      ...
      for(let x = 0; x<body.data.length; x++) {
        ...
        let phone = body.data[x].practices[0].phones.number;
        let patients = body.data[x].practices[0].accepts_new_patients;

        let doctor = new Doctor(firstName, lastName, address, phone, patients);
